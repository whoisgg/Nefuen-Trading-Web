...
function CameraRig() {
  ...
  useFrame((state, delta) => {
    ...
    if (progress < 0.333) {
      ...
      radius = THREE.MathUtils.lerp(15, isMobile ? 5 : 8, ease)
      y = THREE.MathUtils.lerp(5, isMobile ? 1 : 2, ease)
      lookX = THREE.MathUtils.lerp(0, 3, ease)
      lookY = THREE.MathUtils.lerp(0, 0, ease)
    } else if (progress < 0.666) {
      const t = (progress - 0.333) / 0.333
      const ease = t * t * (3 - 2 * t)
      angle = THREE.MathUtils.lerp(Math.PI * 0.5, Math.PI, ease)
      radius = THREE.MathUtils.lerp(isMobile ? 5 : 8, isMobile ? 6 : 6, ease)
      y = THREE.MathUtils.lerp(isMobile ? 1 : 2, 1, ease)
      lookX = THREE.MathUtils.lerp(3, isMobile ? 0 : -3, ease)
      lookY = THREE.MathUtils.lerp(0, 0, ease)
    } else {
